...
- 指・つま先の編集: VRM / Unity Humanoid 共通の指 15 関節 × 両手(親指 3 + 4 指 × 3)と leftToes / rightToes を対象にする
  - 手首 = 手ボーン(leftHand / rightHand)、足首 = 足ボーン(leftFoot / rightFoot)の回転として編集する。IK 中も手首・足首の回転は IK ターゲットの向きとして独立に編集できる
  - 手モード(左パネル「手」タブ / H キー): 手のピッカーで手首 + 15 関節を選んで個別に回転
  - 指全体は「曲げ / 開き」スライダー(Unity の Muscle と同じ意味)で動かす。関節単位は微調整用
  - プリセット(開く / 握る / 指差し / ピース / 親指立て / OK)、右手へのミラーコピー、区間への適用
  - タイムラインでは手全体を 1 行にまとめ、展開すると指ごとの行になる
...
- 表情(ブレンドシェイプ)の編集: 2 層で持つ
  - プリセット行: `.anim`(`blendShape.*` カーブ)や VRM Expression プリセットを「名前付きの値の組」として読み込み、ステップキーで切り替える。隣のキーとはクロスフェード(既定 6 フレーム)
  - 個別カーブ行: ブレンドシェイプごとの 0〜1.0 カーブ(ベジェ / リニア / ステップ)。プリセット行の上に上書きで重なる(口パク・まばたき・微調整)
  - プリセット行はいつでも個別カーブに展開(ベイク)できる
  - `.anim` の読み込みは Unity YAML をパースし、`blendShape.<名前>` を今のアバターのブレンドシェイプ名で照合する。不一致は警告表示
  - 補助: まばたきの自動挿入、音声からのリップシンク(将来)
  - 書き出し: VRMA は Expression、FBX はモーフアニメ、`.anim` は `blendShape.*`。Pose2Clip の `.p2cface` と互換
